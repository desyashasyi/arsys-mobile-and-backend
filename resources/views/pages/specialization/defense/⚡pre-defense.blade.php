...
new #[Layout('layouts.mobile-app')] #[Title('Pre-Defense Events')] class extends Component
{
    public function with(): array
    {
        ...
        $events = Event::with(['program'])
            ...
            ->withCount('defenseApplicant as applicant_count')
            ...
            ->map(fn($e) => [
                ...
                'event_label'      => 'PRE-' . Carbon::parse($e->event_date)->format('dmy') . '-' . $e->id,
                'applicant_count'  => $e->applicant_count,
                'unpublished_count'=> $e->unpublished_count ?? 0,
...
<div>
    <p class="px-4 py-2 text-xs text-purple-600 font-medium">Manage participants &amp; schedules</p>
            ...
            <div class="rounded-2xl bg-white shadow-sm overflow-hidden">
                ...
                <div class="divide-y divide-gray-100">
                    @foreach($dayEvents as $event)
                        @if($event['is_own'])
                            {{-- Own program: prominent teal row, clickable --}}
                            <a href="{{ route('specialization.defense.pre-defense.event', $event['id']) }}"
                               class="flex items-center gap-3 px-4 py-3.5 hover:bg-teal-50 active:bg-teal-100 transition-colors">
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center gap-1.5 mb-0.5">
                                        <span class="text-[9px] font-bold px-2 py-0.5 rounded-full bg-teal-100 text-teal-700">PRE</span>
                                        <span class="text-[10px] font-bold text-teal-700">{{ $event['program_code'] }}</span>
                                    </div>
                                    ...
                                    <div class="flex items-center gap-3 mt-1">
                                        ...
                                        @if($event['unpublished_count'] > 0)
                                            <span class="text-[10px] font-bold text-amber-600">
                                                {{ $event['unpublished_count'] }} unpublished
                                            </span>
                                        @else
                                            <span class="text-[10px] font-bold text-green-600">All published</span>
                                        @endif
                                    </div>
                                </div>
                        ...
                        @else
                            {{-- Other program: compact gray row, clickable for read-only view --}}
                            <a href="{{ route('specialization.defense.pre-defense.event', $event['id']) }}"
                               class="flex items-center gap-3 px-4 py-2.5 bg-gray-50/50 hover:bg-gray-100 transition-colors">
                                <span class="shrink-0 text-[9px] font-bold px-1.5 py-0.5 rounded bg-gray-200 text-gray-500">PRE</span>
                                <div class="flex-1 min-w-0">
                                ...
                                <x-icon name="o-chevron-right" class="h-3.5 w-3.5 text-gray-300 shrink-0" />
                            </a>
                        @endif
                    @endforeach
                </div>
