...
new #[Layout('layouts.mobile-app')] #[Title('Being Reviewed')] class extends Component
{
    use WithPagination;
    ...
    public function with(): array
    {
        ...
        $items = Research::with(['student.program', 'milestone', 'reviewers.staff', 'reviewers.decision', 'milestone'])
            ...
            ->through(fn($r) => [
                ...
                'reviewer_count'  => $r->reviewers->count(),
                'decided_count'   => $r->reviewers->filter(fn($rv) => $rv->decision !== null)->count(),
...
<div>
    <p class="px-4 pt-2 text-xs text-purple-600 font-medium">Proposals under reviewer assessment</p>
    ...
    <div class="px-3 pb-3 space-y-2">
        @forelse($items as $item)
            <a href="{{ route('specialization.research.show', $item['id']) }}"
               class="block rounded-xl bg-white shadow-sm overflow-hidden border-l-4 border-orange-400 p-4 hover:shadow-md transition-shadow">
                <div class="flex items-start justify-between gap-2">
                    ...
                    <div class="shrink-0 flex flex-col items-end gap-1">
                        <span class="text-[9px] font-bold px-2 py-0.5 rounded-full bg-orange-100 text-orange-700">
                            {{ $item['decided_count'] }}/{{ $item['reviewer_count'] }}
                        </span>
        ...
        @empty
            <div class="py-16 text-center">
                ...
                <p class="text-sm text-gray-400">No proposals under review.</p>
            </div>
        @endforelse
    </div>
