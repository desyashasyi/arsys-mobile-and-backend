...
new #[Layout('layouts.mobile-app')] #[Title('In Progress')] class extends Component
{
    use WithPagination;
    ...
    public function with(): array
    {
        ...
        $items = Research::with(['student.program', 'milestone', 'supervisor.staff'])
            ...
            ->whereHas('supervisor')
            ->whereHas('milestone', fn($q) => $q->whereIn('code', ['Pre-defense', 'Final-defense', 'Seminar']))
...
<div>
    <p class="px-4 pt-2 text-xs text-purple-600 font-medium">Pre-defense, Final-defense &amp; Seminar phase</p>
    ...
    <div class="px-3 pb-3 space-y-2">
        @forelse($items as $item)
            <a href="{{ route('specialization.research.show', $item['id']) }}"
               class="block rounded-xl bg-white shadow-sm overflow-hidden border-l-4 border-green-400 p-4 hover:shadow-md transition-shadow">
        ...
        @empty
            <div class="py-16 text-center">
                ...
                <p class="text-sm text-gray-400">No research in progress.</p>
            </div>
        @endforelse
    </div>
