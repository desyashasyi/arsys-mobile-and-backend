...
new #[Layout('layouts.mobile-app')] #[Title('Participant Detail')] class extends Component
{
    ...
    public function with(): array
    {
        ...
        $app = EventApplicantFinalDefense::where('id', $this->applicantId)
            ->whereHas('room', fn($q) => $q
                ->where('event_id', $this->eventId)
                ->whereHas('event', fn($q2) => $q2
                    ->where('program_id', $programId)
                    ->whereHas('type', fn($q3) => $q3->where('code', 'PUB'))))
            ->with([
                'room.examiner.staff',
                'room.examiner.finaldefenseExaminerPresence',
                'research.student.program',
                'research.milestone',
                'research.supervisor.staff',
                'research.supervisor.finaldefenseSupervisorPresence',
            ])
            ->first();
        ...
        $examiners = $room->examiner->map(function ($ex) use ($app, $supervisorMap) {
...
<div>
    <p class="px-4 py-2 text-xs text-purple-600 font-medium">Final Defense Score Detail</p>
        ...
        <div class="px-3 py-3 space-y-3">
            ...
            @if($applicant['examiners']->isNotEmpty())
                <div class="rounded-xl bg-white shadow-sm p-4">
                    <p class="text-xs font-bold text-purple-700 uppercase tracking-wide mb-3">Examiner Scores</p>
                    <div class="space-y-2">
